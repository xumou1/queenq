...
def update_view(view_type, selected_node, current_elements):
    print(f"视图切换回调触发，view_type: {view_type}, selected_node: {selected_node}")
    
    # 获取表格数据
    if selected_node:
        # 如果选中了节点，查询与该节点相关的所有关系
        query = f"""
    ...
    if view_type == "graph":
        if current_elements and len(current_elements) > 0:
            ...
            WITH initial_nodes AS (
                SELECT unique_node_id, canonical_name
                FROM nodes
            ),
            node_edges AS (
                SELECT DISTINCT
                ...
                LEFT JOIN edges e ON n.unique_node_id IN (e.source_node_id, e.target_node_id)
                WHERE e.source_node_id IS NOT NULL AND e.target_node_id IS NOT NULL
            )
            SELECT 
                ne.unique_node_id,
                ne.canonical_name,
...
def update_company_relationships(selected_node):
    if not selected_node:
        return "请选择一个公司查看关系", []
    if not check_db_connection():
        return "数据库连接异常，请刷新页面重试", []
    print(f"查询公司关系，node_id: {selected_node}")
    try:
        with get_conn() as conn:
            query = f"""
            WITH company_relationships AS (
            ...
            print("公司关系查询结果：", df)
            if df.empty:
                print(f"edges表未找到该公司({selected_node})的关系，检查nodes表是否存在该公司...")
                node_check = conn.execute(f"SELECT * FROM nodes WHERE unique_node_id = '{selected_node}'").df()
                if node_check.empty:
                    print(f"nodes表也未找到该公司({selected_node})，数据可能确实缺失。")
                else:
                    print(f"nodes表存在该公司({selected_node})，但edges表无关系。")
                return "未找到公司关系", []
            elements = []
            nodes = set()
            ...
            elements.append({"data": {"id": selected_node, "label": center_name}})
            nodes.add(selected_node)
            for _, row in df.iterrows():
                if row['connected_node_id'] not in nodes:
                    elements.append({"data": {"id": row['connected_node_id'], "label": row['connected_company']}})
